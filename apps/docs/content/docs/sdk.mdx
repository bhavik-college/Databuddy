...
Track error events with additional context. This is a convenience wrapper around `track("error", ...)`.

<Callout type="info">
  **Note:** `trackError()` is for *manual* error tracking. To automatically capture all JavaScript errors, use the `trackErrors` prop on the `<Databuddy />` component instead.
</Callout>
...
```javascript
import { track, trackError, isTrackerAvailable } from "@databuddy/sdk";
...
track<CustomEventName>("signup_completed", {
  method: "email",
  source: "landing_page"
});
```
